...
for cat in ['cs.AI','cs.LG','cs.CL']:
    url = f"http://export.arxiv.org/api/query?search_query=cat:{cat}&sortBy=submittedDate&sortOrder=descending&max_results=50"
    try:
        r = requests.get(url, timeout=30)
        print(f"Status for {cat}: {r.status_code}")
        root = ET.fromstring(r.content)
        entries = root.findall('atom:entry', ns)
        print(f"Entries for {cat}: {len(entries)}")
        for entry in entries:
            ...
            if score >= 2:
                ...
                print(f"  MATCH: {arxiv_id} | {date} | {title[:60]}... | score={score}")
